...
def create_conversation(req: Optional[ConversationCreateRequest] = Body(None)):
    """创建新对话"""
    db = get_db()
    try:
...
def delete_conversation(conversation_id: int, user_id: int = 0):
    """删除单个对话及其所有消息"""
    db = get_db()
    try:
        # 删除关联的转人工请求
        db.execute("DELETE FROM transfer_requests WHERE conversation_id = %s", (conversation_id,))
        # 删除对话中所有消息
...
def clear_all_conversations(user_id: int):
    """清除用户所有对话"""
    db = get_db()
    try:
        # 获取该用户所有对话ID
        ...
        if conv_ids:
            ...
            db.execute(
                f"DELETE FROM transfer_requests WHERE conversation_id IN ({placeholders})",
                tuple(conv_ids),
            )
            # 删除所有消息
            db.execute(
        ...
        db.commit()
        return {"message": f"已清除 {len(conv_ids)} 条对话"}
    finally:
        db.close()
